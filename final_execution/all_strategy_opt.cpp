...
vl cnt_per_row,cnt_per_column;
ll cnt_diag0,cnt_diag1;
...
ll xcount(ll i,ll j,vector<vector<char> > &board){
	...
	if(board[i][j]=='x'){
		...
		// cout<<cnt_per_row[i]<<" "<<cnt_per_column[j]<<" "<<cnt_diag0<<" "<<cnt_diag1<<endl;
		val=val+cnt_per_row[i];
		val=val+cnt_per_column[j];
		if(i==j){
			val=val+cnt_diag0;
		}
		if(i==n-1-j){
